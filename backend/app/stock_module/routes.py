from app import database, helpers, stock_module
...
async def get_all_stocks(db: Database = Depends(database.get_db)):
    ...
    stock_data = stock_module.services.get_all_stocks(db)
...
async def get_stock_by_ticker(ticker: str, db: Database = Depends(database.get_db)):
    ...
    ticker = ticker.upper()
    stock_data = stock_module.services.get_stock_by_ticker(db, ticker)
    return {"data": stock_data}
